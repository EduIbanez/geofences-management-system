...
public class UserController {
    ...
    public ResponseEntity<User> createUser(@RequestBody User user) {
        ...
        if (created) {
            return new ResponseEntity<User>(new User(user.getId(), user.getEmail(), user.getPass(), user.getFirst_name(),
    ...
    public ResponseEntity<User> deleteUser(@PathVariable("id") int id) {
        ...
        if (found) {
            if (deleted) {
                return new ResponseEntity<User>(new User(id, "", "", "", "", "", ""), HttpStatus.OK);
            }
            else {
                return new ResponseEntity<User>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
    ...
    public ResponseEntity<User> loginUser(@RequestParam("email") String email, @RequestParam("pass") String pass) {
        ...
        if (checked) {
            return new ResponseEntity<User>(new User(1, email, pass, "", "", "", ""), HttpStatus.OK);
        }
        else {
            return new ResponseEntity<User>(HttpStatus.NOT_FOUND);
        }
